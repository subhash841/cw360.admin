...
class ApnsPHP_Push_Server extends ApnsPHP_Push {
	...
	switch ($nSignal) {
		...
		if (($nPid = posix_getpid()) != $this->_nParentPid) {
		    ...
		    $this->messages = "INFO: Child $nPid received signal #{$nSignal}, shutdown...~~";
		    $this->_nRunningProcesses--;
		    exit(0);
		}
		break;
		...
		$this->messages = "INFO: Ignored signal #{$nSignal}.~~";
		break;
	}
    }
	...
	if (posix_getpid() == $this->_nParentPid) {
	    ...
	    $this->messages = "INFO: Parent shutdown, cleaning memory...~~";
	...
	for ($i = 0; $i < $this->_nProcesses; $i++) {
		...
		$this->messages = "WARNING: Could not fork~~";
	    } else if ($nPid > 0) {
		// Parent process
		$this->_log("INFO: Forked process PID {$nPid}");
		$this->messages = "INFO: Forked process PID {$nPid}~~";
		$this->_nRunningProcesses++;
	    } else {
		// Child process
		try {
		    ...
		    $this->messages = "ERROR: " . $e->getMessage() . ", exiting...~~";
		    exit(1);
		}
		$this->_mainLoop();
		parent::disconnect();
	...
	while (true) {
		...
		$this->messages = "INFO: Parent process {$this->_nParentPid} died unexpectedly, exiting...~~";
		break;
	    }

	    sem_acquire($this->_hSem);
	    $this->_setQueue(self::SHM_ERROR_MESSAGES_QUEUE_KEY, 0, array_merge($this->_getQueue(self::SHM_ERROR_MESSAGES_QUEUE_KEY), parent::getErrors())
	    );
		...
		$this->messages = "INFO: Process " . ($this->_nCurrentProcess + 1) . " has {$nMessages} messages, sending...~~";
		parent::send();
	    } else {
