...
class RatedArticle extends Base_Controller {
    ...
    function index() {
        ...
        if ($article_id != 0) {
            $data = $this->RatedArticle_model->get_article_details($article_id);
        }
    ...
    function lists() {
        ...
        $data['articles'] = $this->RatedArticle_model->get_list();
    ...
    function active_inactive_article() {
        $inputs = $this->input->post();

        $this->RatedArticle_model->active_inactive_article_mod($inputs);

        $message = ($inputs['current'] == "1") ? "Article deactivated successfully" : "Article activated successfully";
