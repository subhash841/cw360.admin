...
class ApnsPHP_Message {
    ...
    public function __construct($sDeviceToken = null) {
        if (isset($sDeviceToken) && is_array($sDeviceToken)) {
            foreach ($sDeviceToken as $DeviceToken) {
                $this->addRecipient($DeviceToken);
            }
        } else if (isset($sDeviceToken)) {
    ...
    protected function _getPayload() {
        ...
        if (isset($this->_sText) && is_array($this->_sText)) {
    ...
    public function getPayload() {
        ...
        if (!defined('JSON_UNESCAPED_UNICODE') && function_exists('mb_convert_encoding')) {
            $sJSON = preg_replace_callback(
                    '~\\\\u([0-9a-f]{4})~i', create_function('$aMatches', 'return mb_convert_encoding(pack("H*", $aMatches[1]), "UTF-8", "UTF-16");'), $sJSON);
        }

        $sJSONPayload = str_replace(
                '"' . self::APPLE_RESERVED_NAMESPACE . '":[]', '"' . self::APPLE_RESERVED_NAMESPACE . '":{}', $sJSON
        );
