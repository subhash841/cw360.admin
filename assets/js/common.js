...
showToast = function (msg, status) {
 ...
 showToast_coins = function(status) {
    if(status == 'add') {
              $("#response_message").html('<div class="btn-lg bg-green"> Coins Added Successfully</div>').fadeIn(2000).delay(1000).fadeOut(2000);
    } else if(status == 'deduct'){ 
              $("#response_message").html('<div class="btn-lg bg-red"> Coins Deducted Successfully</div>').fadeIn(2000).delay(1000).fadeOut(2000);
    } else if(status == 'error'){
             $("#response_message").html('<div class="btn-lg bg-red">Insufficient User Coins</div>').fadeIn(2000).delay(1000).fadeOut(2000);
    } 
 }



 function image_ext(fileType){
...
*/
    ...
    $("#categoryModal").on("shown.bs.modal", function (event) {
        ...
        if (id != "0") {
            $( 'form[name="addUpdateBlogCategory"]' ).validate().resetForm();
            $("#defaultModalLabel").html('Edit Category');
            $("input").focus();
            $('.add-cat-btn').text('SAVE CHANGES');
         
        } else {
            $( 'form[name="addUpdateBlogCategory"]' ).validate().resetForm();
            $("#defaultModalLabel").html('Add Category');
            $("input").blur();
            $('.add-cat-btn').text('SUBMIT');
    ...
    var is_category_submitted = false;
    ...
    $('form[name="addUpdateBlogCategory"]').on('submit', function (event) {
        ...
        if($('input[name="category_name"]').val()  != ""){
            if(!is_category_submitted){
                is_category_submitted = true;
        ajax_call('Blogs/addUpdateBlogCategory', "POST", param, function (result) {
            console.log(result);
            result = JSON.parse(result);
            if (result['status']) {
                showToast(result['message'], '1');
    ...
    $('form[name="addUpdateBlogSubCategory"]').on('submit', function (event) {
        ...
        var param = $(this).serialize();
        //console.log(param);
        ajax_call('Blogs/addUpdateBlogSubCategory', "POST", param, function (result) {
            console.log(result);
            result = JSON.parse(result);
            if (result['status']) {
                showToast(result['message'], '1');
    ...
    $(document).on('change', '.changeactive', function (e) {
        ...
        var status = $(this).attr('data-status');
            ...
            ajax_call('Blogs/changeActive', "POST", param, function (result) {
...
$('form[name="addUpdateState"]').on('submit', function (event) {
    ...
    var param = $(this).serialize();
    //console.log(param);
    ajax_call('Master/addUpdateState', "POST", param, function (result) {
        console.log(result);
        result = JSON.parse(result);
        if (result['status']) {
            showToast(result['message'], '1');
...
$('form[name="addeditparty"]').on('submit', function (event) {
    ...
    //console.log(param);
    ajax_call_multipart('Master/addUpdateParty', "POST", formdata, function (result) {
        console.log(result);
        result = JSON.parse(result);
        if (result['status']) {
            showToast(result['message'], '1');
...
$("#topicModal").on("show.bs.modal", function (event) {
    ...
    if (id == "0") {
        ...
        $('#defaultModalLabel').html('ADD TOPIC');
        ...
        $(base + "[type=submit]").text('SUBMIT');
        $( 'form[name="addUpdateTopic"]' ).validate().resetForm();


    } else {

        $('#defaultModalLabel').html('EDIT TOPIC');
        var data = relatedTarget.attr("data-editjson");
...
// $('.topic_cat').selectpicker(data.category);
        console.log($('.topic_cat option[value=' + data.category + ']').attr('selected', 'selected'));
        $('.topic_cat').selectpicker('render');
        if (is_trending == 1) {
        ...
        $(base + "[type=submit]").text('SAVE CHANGES');
        $( 'form[name="addUpdateTopic"]' ).validate().resetForm();
...
$('form[name="addUpdateTopic"]').on('submit', function (event) {
    ...
    if (validateAllforms("addUpdateTopic", topic_id)) {
        ajax_call('Master/addUpdateTopic', "POST", param, function (result) {
            console.log(result);
            result = JSON.parse(result);
            if (result['status']) {
                showToast(result['message'], '1');
...
/*$('form[name="addUpdateTopic"]').on('submit', function (event) {
    ...
    $(document).on('change', '.changeActiveTopic', function (e) {
        ...
        var status = $(this).attr('data-status');
        var param = {id: id, type: type, current: status};
        ajax_call('Master/changeActiveTopic', "POST", param, function (result) {
    ...
    var a;
    $(document).on('change', '.changepublishgame', function (e) {
         var _this = $(this);
        var id = $(this).attr('data-id');
        var type = $(this).attr('data-type');
        var status = $(this).attr('data-status');
        var param = {id: id, type: type, current: status};
        ajax_call('Games/changepublishgame', "POST", param, function (result) {
    ...
    $(document).on('change', '.changepublishquiz', function (e) {
         var _this = $(this);
        var id = $(this).attr('data-id');
        var type = $(this).attr('data-type');
        var status = $(this).attr('data-status');
        var param = {id: id, type: type, current: status};
        ajax_call('Quiz/changepublishquiz', "POST", param, function (result) {
    ...
    $(document).on('change', '.changepublishprediction', function (e) {
         var _this = $(this);
        var id = $(this).attr('data-id');
        var type = $(this).attr('data-type');
        var status = $(this).attr('data-status');
        var param = {id: id, type: type, current: status};
        ajax_call('Predictions/changepublishprediction', "POST", param, function (result) {
    ...
    $(document).on('change', '.changepublishquestion', function (e) {
        var _this = $(this);
        var id = $(this).attr('data-id');
        var type = $(this).attr('data-type');
        var status = $(this).attr('data-status');
        var param = {id: id, type: type, current: status};
        ajax_call('Quiz/changepublishquestion', "POST", param, function (result) {
            result = JSON.parse(result);
            if (result['status']) {
                showToast(result['message'], '1');
                $(_this).attr('data-status','1');
    ...
    $(document).on('change', '.changepublishreward', function (e) {
        var _this = $(this);
        var id = $(this).attr('data-id');
        var type = $(this).attr('data-type');
        var status = $(this).attr('data-status');
        // alert(status);
        var param = {id: id, type: type, current: status};
        ajax_call('Reward/changepublishreward', "POST", param, function (result) {
...
$(document).on('change', '.changeTrendingTopic', function (e) {
    ...
    var param = {id: id, type: type, current: status};
    ajax_call('Master/changeTrendingTopic', "POST", param, function (result) {
...
function validateAllforms(type, id) {
    ...
    if (type == "addUpdateTopic") {
        ...
        if(($(forname + ' select[name="blog_id"]').find(":selected").val()) == "" ){
            isValid = false;
            console.log($('#desErr'));
            $('#desErr').html('This field is required.');
            // console.log('invalid');
        }
...
$("#userModal").on("show.bs.modal", function (event) {
    ...
    if (id == "0") {
        ...
        $(".passwor").show();
        $('#defaultModalLabel').html('ADD');
        $(base + "[name='user_id']").val("0");
        $(base + "[name='admin_email']").val('');
        $(base + "[name='admin_password']").val('');
        $(base + "[name='admin_role']").val('');
        $(base + "[name='is_active']").val('');
        $(base + "[name='menu_list']").val('');
        $(".new-user-btn").text('SUBMIT');
    } else {

        $('#defaultModalLabel').html('EDIT');
        $(".new-user-btn").text('SAVE CHANGES');
        var data = relatedTarget.attr("data-editjson");
        data = JSON.parse(data);
        //console.log(data);
        var id = data.id;
        ...
        $(base + "input[name='user_id']").val(id);
        $(base + "input[name='admin_email']").val(email);
        if (is_active == 1) {
            $(base + "input[name='is_active']").prop('checked', true);
        } else {
            $(base + "input[name='is_active']").prop('checked', false);
        }
        $(base + "select[name='admin_role']").val(role);
        $('select[name="admin_role"]').selectpicker('refresh');

        $(base + "input[name='is_active']").val('');    
        $(base + "input[name='menu_list']").val('');


        $(".menu-lists input[type='checkbox']").each(function(){
        ...
        if(data.menu_list != ""){
            var menuListArray = data.menu_list.split(",");
            //console.log(menuListArray);
            $(".menu-lists").find('[value=' + menuListArray.join('], [value=') + ']').prop("checked", true);
        }
    }
});
...
$('form[name="addUpdateUser"]').on('submit', function (event) {
    event.preventDefault();
    ...
    $.each($("input[name='menu_list[]']:checked"), function(){
                menu_list.push($(this).val());
    });

    if(admin_email != '' && admin_pas != '' && menu_list == ''){
            showToast('Please Select Menu List','2');
    }
    
    var param = $(this).serialize();
   
                ajax_call('Master/addUpdateUser', "POST", param, function (result) {

                            result = JSON.parse(result);
                            if(result.status == 'failure' ){
                                     $.each(result.error, function (index, value) {

                                        if (index == 'admin_email') {
                                                $('.admin_emailErr').text(result.error.admin_email).css("color","#f44336");
                                        }else if(index == 'admin_password'){
                                                $('.admin_passwordErr').text(result.error.admin_password).css("color","#f44336");
                                        }
...
$(document).on('change', '.changeActiveUser', function (e) {

    var id = $(this).attr('data-id');
    var type = $(this).attr('data-type');
    var status = $(this).attr('data-status');
    var param = {id: id, type: type, current: status};
    ajax_call('Master/changeActiveUser', "POST", param, function (result) {
