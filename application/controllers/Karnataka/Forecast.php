...
//    }
    ...
    function reasons(){
        $page_title['page_title'] = "Karnataka/reasons";
        $data['reasons']= $this->Forecast_model->get_forecast_reasons(2);
        $this->load->view('template/header', $page_title);
        $this->load->view('Karnataka/reasons', $data);
        $this->load->view('template/footer');
    }
    
    function deleteforecastreason(){
        $id= $this->input->post('forecastreasonid');
        $this->Forecast_model->deletereason_mod($id);
