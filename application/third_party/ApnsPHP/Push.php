...
class ApnsPHP_Push extends ApnsPHP_Abstract
{
	...
	public function send()
	{
		...
		while (($nMessages = count($this->_aMessageQueue)) > 0) {
			...
			$this->messages = "INFO: Sending messages queue, run #{$nRun}: $nMessages message(s) left in queue.~~";
			...
			foreach($this->_aMessageQueue as $k => &$aMessage) {
				...
				if (!empty($aMessage['ERRORS'])) {
					foreach($aMessage['ERRORS'] as $aError) {
						if ($aError['statusCode'] == 0) {
							...
							$this->messages = "INFO: Message ID {$k} {$sCustomIdentifier} has no error ({$aError['statusCode']}), removing from queue...~~";
							...
							$this->messages = "WARNING: Message ID {$k} {$sCustomIdentifier} has an unrecoverable error ({$aError['statusCode']}), removing from queue without retrying...~~";
					...
					if (($nErrors = count($aMessage['ERRORS'])) >= $this->_nSendRetryTimes) {
						...
						$this->messages = "WARNING: Message ID {$k} {$sCustomIdentifier} has {$nErrors} errors, removing from queue...~~";	
						$this->_removeMessageFromQueue($k, true);
						continue;
					}
				}
				...
				$this->messages = "STATUS: Sending message ID {$k} {$sCustomIdentifier} (" . ($nErrors + 1) . "/{$this->_nSendRetryTimes}): {$nLen} bytes.~~";
			...
			if (!$bError) {
				...
				if ($nChangedStreams === false) {
					...
					$this->messages = "ERROR: Unable to wait for a stream availability.~~";
					break;
	...
	protected function _updateQueue($aErrorMessage = null)
	{
		...
		$this->messages = "ERROR: Unable to send message ID ".$aErrorMessage['identifier'] . ":" .$aErrorMessage['statusMessage'] . " (" . $aErrorMessage['statusCode'] . ").~~";
